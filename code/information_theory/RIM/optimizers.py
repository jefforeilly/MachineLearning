...
def gradient_descent(f, starting_point, learning_rate=0.1):
    ...
    gradient, which in turn is obtained by gradient polynomial expansion (see
    param f)

    :param f: ExpPoly2D object calculating approimate derivatives based on
    gradient expansion interpolation (or any object returning gradient via gradient
    ...
    while True:
        output, gradient = f.gradient(theta)
        yield theta, output, gradient
        theta -= learning_rate * gradient


def momentum(f, starting_point, learning_rate=0.1, gamma=0.9):
    ...
    :param f: ExpPoly2D object calculating approimate derivatives based on
    gradient expansion interpolation
...
def nesterov(f, starting_point, learning_rate=0.1, gamma=0.9):
    ...
    while True:
        ...
        _, estimation_gradient = f.gradient(estimation)
        accumulated_gradient = gamma * accumulated_gradient \
            + learning_rate * estimation_gradient.reshape(theta.shape)
        theta -= accumulated_gradient


def adagrad(f, starting_point, learning_rate=0.1, epsilon=1e-8):
...
def rmsprop(f, starting_point, learning_rate=1e-3, gamma=0.9, epsilon=1e-8):
    ...
    :param f: ExpPoly2D object calculating approimate derivatives based on
    gradient expansion interpolation. See source code for more information
    :param starting_point: initial theta parameters (for example randomly
    initialized)
    :param learning_rate: Learning rate for the algorithm
    :param gamma: Multiplier for previous gradient (size of an effect it has
